    ...
    return plugin.route({
      ...
      handler: function(request, reply) {
        ...
        if (!(request.payload.users && _.isArray(request.payload.users) && request.payload.users.length > 0)) {
          return reply(Boom.badRequest(i18n.errorAtLeastOneUserRequired));
        }
        if (options.secretKey && options.secretKey !== request.payload.secretKey) {
          return reply(Boom.badRequest(i18n.secretKeyMismatch));
        }
